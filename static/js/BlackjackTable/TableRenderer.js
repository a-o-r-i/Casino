...
export function CreateTableRenderer({
  ...
  function GetPlayerCardScale() {
    return UsesSharedCardMetrics() ? PLAYER_CARD_SCALE : 1;
  }
  ...
  function GetPlayerLayout(SeatId, HandIndex = 0, HandCount = 1) {
    ...
    if (Slot) {
      ...
      const PlayerCardScale = UseSharedMetrics ? PLAYER_CARD_SCALE : 1;
    ...
    if (!Seat) {
      const FallbackMetrics = ScaleCardMetrics(GetCardMetrics(), GetPlayerCardScale());
      return {
        anchor: {
          x: StageRect.width / 2,
  ...
  function GetPlayerStackStep(Slot, StageRect, Metrics, HandCount = 1) {
    const PlayerCardScale = GetPlayerCardScale();
    const BaseStep = Slot ? {
      x: ParsePercent(Slot.stackOffsetX, StageRect.width) * PlayerCardScale,
      y: ParsePercent(Slot.stackOffsetY, StageRect.height) * PlayerCardScale
    } : {
      x: Clamp(Metrics.width * CARD_STACK_STEP_X_RATIO, 36, 44),
      y: Clamp(Metrics.height * 0.16, 8, 12)
    };
  ...
  function RenderSeatBetSpots(State, View) {
    ...
    SEAT_POSITIONS.forEach(Seat => {
      ...
      ["perfect_pairs", "twenty_one_plus_three"].forEach(BetType => {
        ...
        const HasSideBetValue = Number(Spot.amount) > 0;
        const HasWinningResolvedSideBet = Number(Spot.payout) > 0 || Boolean(Spot.payoutLabel);
        const HasVisibleSideBet = ShouldShowForBetting ? HasSideBetValue : HasWinningResolvedSideBet;
        const IsEditorSelected = SelectedEditorKey === SideBetKey(Seat.id, BetType);
        const ShouldShow = EditorActive || (IsClaimed && ShouldShowForBetting) || HasVisibleSideBet;
